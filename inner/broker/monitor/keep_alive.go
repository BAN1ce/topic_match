...
func (k *KeepAlive) Start(ctx context.Context) error {
	...
	for {
		select {
		case <-ctx.Done():
			logger.Logger.Info().Msg("keepalive monitor exit by context done")
			return nil
		case t, ok := <-k.tinker.C:
			if !ok {
				logger.Logger.Info().Msg("keepalive monitor exit by tinker close")
				return nil
			}
			k.mux.RLock()
			...
			if err != nil && !errors.Is(err, nutsdb.ErrBucket) {
				logger.Logger.Error().Err(err).Msg("keepalive monitor error")
			}
			if k.expiredFunc != nil && len(uid) > 0 {
				logger.Logger.Debug().Strs("uid", uid).Msg("client keep alive expired, do expired function")
				k.expiredFunc(uid)
			}
		}
...
func (k *KeepAlive) SetClientAliveTime(uid string, t *time.Time) error {
	...
	defer cancel()
	logger.Logger.Debug().Str("uid", uid).Msg("set client alive time")
